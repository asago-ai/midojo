...
- **Channel** (`src/midojo/channels.py`): where a probe's payload enters the agent. A probe with no `channel` is delivered by placeholder substitution (the default). A probe that names one — `tool_output`, `tool_description` — becomes an instruction on the evaluation's *injection plan* for an interception adapter to deliver, and is deliberately kept out of the substitution path so it is never delivered twice
