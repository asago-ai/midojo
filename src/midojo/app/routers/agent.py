...
def get_environment(token: Token, store: Storage) -> dict:
...
def put_environment(
    body: dict, token: Token, store: Storage, catalog: Annotated[SuiteCatalog, Depends(get_catalog)]
) -> dict:
    with store.session(token) as evaluation:
...
def record_function_call(req: CreateFunctionCallRecord, token: Token, store: Storage) -> FunctionCallResponse:
    with store.session(token) as evaluation:
...
def list_function_calls(token: Token, store: Storage) -> list[FunctionCallResponse]:
    with store.session(token) as evaluation:
...
def get_function_call(idx: int, token: Token, store: Storage) -> FunctionCallResponse:
    with store.session(token) as evaluation:
...
def record_observations(req: RecordObservationsRequest, token: Token, store: Storage) -> dict:
    with store.session(token) as evaluation:
...
def get_observations(token: Token, store: Storage) -> dict:
    with store.session(token) as evaluation:
        return dict(evaluation.observations)
